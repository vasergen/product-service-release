...
if kubectl --context="${CLUSTER_NAME}" -n "${NAMESPACE}" create configmap "${CONFIGMAP_NAME}" --from-literal="${KEY}=${NEXT_DB}" --dry-run=client -o yaml | kubectl --context="${CLUSTER_NAME}" apply -f -; then
  FINAL_DB=$(kubectl --context="${CLUSTER_NAME}" -n "${NAMESPACE}" get configmap "${CONFIGMAP_NAME}" -o jsonpath="{.data.${KEY}}" 2>/dev/null)
